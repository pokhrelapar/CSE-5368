...
from Pokhrel_01_01 import multi_layer_nn
...
def create_toy_data_nonlinear_2d(n_samples=1000):
    ...
    y[:, 0] = (
        0.5 * X[:, 0] - 0.2 * X[:, 1] ** 2 - 0.2 * X[:, 2] + X[:, 3] * X[:, 1] - 0.1
    )
...
def test_check_weight_update():
    ...
    np.random.seed(1234)
    [W_before, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [2, 2], alpha=0.2, epochs=0, h=1e-8, seed=1234
    )
    np.random.seed(1234)
    [W_after, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [2, 2], alpha=0.2, epochs=1, h=1e-8, seed=1234
    )
    delta1 = W_after[0] - W_before[0]
    delta2 = W_after[1] - W_before[1]

    correct_delta1 = np.array(
        [
            [-0.00429947, -0.12501811, -0.12501811, -0.12501811, -0.12501812],
            [0.05919799, 0.13948422, 0.13948422, 0.13948422, 0.13948422],
        ]
    )
    correct_delta2 = np.array(
        [[-0.23519989, -0.17988833, -0.21721119], [0.37353127, 0.08978976, 0.04998413]]
    )
...
def test_large_alpha_test():
    ...
    [W, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [2, 1], alpha=10, epochs=1, h=1, seed=2
    )
    assert err[-1] > 0.3
...
def test_small_alpha_test():
    ...
    [W, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [2, 1], alpha=1e-9, epochs=10, h=1e-8, seed=2
    )
...
def test_number_of_nodes_test():
    ...
    [W, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [100, 1], alpha=1e-9, epochs=0, h=1e-8, seed=2
    )
    ...
    [W, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [42, 1], alpha=1e-9, epochs=0, h=1e-8, seed=2
    )
    ...
    [W, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [42, 2], alpha=1e-9, epochs=0, h=1e-8, seed=2
    )
    assert W[0].shape == (42, 5)
...
def test_h_value_used():
    ...
    np.random.seed(1234)
    [W_after, err, Out] = multi_layer_nn(
        X_train, Y_train, X_test, Y_test, [2, 2], alpha=0.2, epochs=1, h=10, seed=1234
    )
